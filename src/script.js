class Animation {
  ...
  constructor(selector) {
    ...
    this.svgWidth =
      this.cols * this.circleSize + 2 * this.svgMargin + (this.cols - 1) * this.spacingHorizontal;
  ...
  setCircleScale() {
    ...
    this.circles.forEach((circle) => {
      ...
      const distance = Math.sqrt(
        Math.pow(circle.center.x - this.mouse.x, 2) + Math.pow(circle.center.y - this.mouse.y, 2)
      );
...
window.onscroll = function (event) {
  ...
  if (scroll >= 0 && scroll < 1700) {
    body.style.backgroundColor = "#FFDDB1";
  } else if (scroll >= 4000 && scroll < 9900) {
    body.style.backgroundColor = "#FCE6FD";
